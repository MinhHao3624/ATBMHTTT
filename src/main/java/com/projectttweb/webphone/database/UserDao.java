...
//		System.out.println(us.getPassWord());
    ...
    public User selectById2(User user) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                // Đã xóa gọi rs.getString("soDu") và rs.getString("typeuser")

                us = new User(userID, userName, passWord, fullName, email, phoneNum, role, dateOfBirth, sex, addRess,
                        date, maXacNhan, thoiGianXacNhan, status, image, key, "0", "Khách Hàng");
                break;
            }
            JDBCUtil.closeConnection(con);
        } catch (Exception e) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                us = new User(userID, userName, passWord, fullName, email, phone, new Roles(roleID, "Khách Hàng"),
                        dateOfBir, sex, addRess, createAt, maXacNhan, thoiGianXacNhan, status, img, key, "0", "Khách Hàng");
                System.out.println("okok");
                break;
            }
            JDBCUtil.closeConnection(con);
    ...
    public int capNhatSoDuMoi(User user, String menhGia) {
        ...
        int res = 0;
        try {
			/* Comment lại toàn bộ logic vì không có trường soDu trong database
			Connection con = JDBCUtil.getConnection();
    ...
    public String getSoDu(String userID) {
        // TODO Auto-generated method stub
        String ans = "0"; // Trả về mặc định là 0 vì database không còn lưu số dư
        try {
			/* Comment lại việc truy vấn soDu để tránh lỗi
			Connection con = JDBCUtil.getConnection();
			String sql = "SELECT * FROM user WHERE userID = ?";
			PreparedStatement stm = con.prepareStatement(sql);
